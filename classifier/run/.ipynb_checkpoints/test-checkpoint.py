...
from run.utility import get_energy_dict, loss_plot, rmsd_scatter, e_rmsd_scatter, type_histo

def add_test_specific_args(parser):
    parser.add_argument("-e_list", help="List of structures and their energy.")
    parser.add_argument("-title", help="Title for the generated plots.")
    return parser

@th.no_grad()
def test(model, loader, title, device, e_list = "", cutoff = [10, 5]):
    model.eval()
    max_label = 0
    max_loss = 0
    max_pred = 0
